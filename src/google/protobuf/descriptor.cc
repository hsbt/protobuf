...
#define PROTOBUF_DEFINE_OPTIONS_ACCESSOR(CLASS, TYPE)              \
  const TYPE& CLASS::options() const {                             \
    const TYPE* result = options_;                                 \
    return result == nullptr ? TYPE::default_instance() : *result; \
  }
      ...
      internal::VisitDescriptors(
          *result, proto, [&](const auto& descriptor, const auto& proto) {
            using OptionsT = std::remove_const_t<
                typename decltype(descriptor.options_)::value_type>;
            using DescriptorT =
            ...
            ResolveFeatures(
                ...
                const_cast<  // NOLINT(google3-runtime-proto-const-cast)
                    OptionsT*>(descriptor.options_.get()),
                alloc);
          });
    }
